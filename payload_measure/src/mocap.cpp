...
int main(int argc, char **argv)
{
  ...
  ros::Publisher point_pub = nh.advertise<geometry_msgs::Point>("point3",2);
  ros::Publisher mtheta_p_pub = nh.advertise<geometry_msgs::Point>("theta_p3",2);
...
//    double apz =a_z_I +alpha*r*cos(forceest1.x[thetap]) + forceest1.x[omegap]*forceest1.x[omegap]*r *sin(forceest1.x[thetap]);
    ...
    theta.x  = ukf2_data.force.x;//forceest1.x[FF_z];
    theta.y  = ukf2_data.force.z; //object.angular_velocity.y;
