...
void pos_cb(const  geometry_msgs::PoseStamped::ConstPtr &msg){
  ...
  double dt = pos.header.stamp.toNSec()- last_time;
  std::cout<<dt<<std::endl;
  vel_diff = (pos.pose.position.x - last_mpos)/0.02;
  last_mpos = pos.pose.position.x;
  last_time = ros::Time::now().toNSec();
}
